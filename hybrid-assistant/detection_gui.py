...
class DetectionGUI:
    ...
    def _init_clip_model(self):
        ...
        if CLIPModel is None or torch is None:
            print("[CLIP] Transformers/Torch nije dostupan.")
            return False
...
Assistant:"""
    ...
    def capture_current_frame(self, context='visual'):
        ...
        if self.vid and self.vid.isOpened():
            ...
            if ret:
                ...
                img_path = os.path.join(self.learning_dir, f"learned_image_{timestamp}.jpg")
                ...
                if context == 'face':
                    if hasattr(self, 'face_cascade') and self.face_cascade is not None:
                        ...
                        if len(faces) == 1:
                            ...
                            face_resized = cv2.resize(face_crop_gray, (200, 200))
                            return img_path, frame, face_resized
                        else:
                            return img_path, frame, None
    ...
    def send_with_frame(self):
        """Capture current frame, attach it to outgoing message, and send."""
        if not self.is_camera_on:
            messagebox.showwarning('Warning', 'Camera is off. Turn it on to attach current frame.')
            return
        img_path, frame, _ = self.capture_current_frame(context='visual')
        if not img_path:
            messagebox.showerror('Error', 'Could not capture frame to attach.')
            return
        self.attached_image_path = img_path
        self.send_message()

    # --- IZMENA (HIBRIDNI MOD): start_add_object ---
    def start_add_object(self):
        """
        Priprema canvas za crtanje PRAVOUGAONIKA (Hibridni mod).
        POBOLJSANJA: Automatski pauzira glavni canvas, Bolji UX
        """
        if self.dialog_canvas_widget is None or not self.is_camera_on:
            messagebox.showwarning("Camera Required", "Camera must be on to use this feature.")
            return
        
        # --- NOVO: Provera konteksta ---
        if self.dialog_context_var and self.dialog_context_var.get() == 'face':
            messagebox.showwarning("Mode Not Supported", 
                "Rectangle drawing is only for General/Visual modes.\nFace mode auto-detects faces.")
            return
            
        self.dialog_toggle_freeze(freeze=True)
        self._cleanup_drawing_tools() 
        self.current_drawing_mode = 'rectangle'
        
        try:
            self.dialog_canvas_widget.config(cursor='crosshair')
            self.dialog_canvas_widget.bind('<ButtonPress-1>', self._on_canvas_press)
            self.dialog_canvas_widget.bind('<B1-Motion>', self._on_canvas_drag)
            self.dialog_canvas_widget.bind('<ButtonRelease-1>', self._on_canvas_release)
            self.dialog_canvas_widget.bind('<Escape>', lambda e: self._cleanup_drawing_tools())
            self.append_to_chat("🔴 Rectangle Mode: Click and drag to draw a box. Press Escape to cancel.")
        except Exception as e:
            print(f"[ADD-OBJ] Failed to bind for rectangle: {e}")
            messagebox.showerror("Error", f"Failed to activate rectangle mode: {e}")
    # --- KRAJ IZMENE ---

    # --- IZMENA (HIBRIDNI MOD): start_polygon_select ---
    def start_polygon_select(self):
        """
        Priprema canvas za crtanje POLIGONA (Hibridni mod).
        POBOLJSANJA: Auto-correct closure, Bolji UX feedback
        """
        if self.dialog_canvas_widget is None or not self.is_camera_on:
            messagebox.showwarning("Camera Required", "Camera must be on to use this feature.")
            return
        ...
        if self.dialog_context_var and self.dialog_context_var.get() == 'face':
            messagebox.showwarning("Mode Not Supported", 
                "Polygon drawing is only for General/Visual modes.\nFace mode auto-detects faces.")
            return

        self.dialog_toggle_freeze(freeze=True)
        self._cleanup_drawing_tools() 
        self.current_drawing_mode = 'polygon'
        self.polygon_points = []

        try:
            self.dialog_canvas_widget.config(cursor='tcross')
            self.dialog_canvas_widget.bind('<ButtonPress-1>', self._on_polygon_click)
            self.dialog_canvas_widget.bind('<Double-Button-1>', self._on_polygon_close)
            self.dialog_canvas_widget.bind('<Motion>', self._on_polygon_motion)
            self.dialog_canvas_widget.bind('<Escape>', lambda e: self._cleanup_drawing_tools())
            self.append_to_chat("🟢 Polygon Mode: Click to add points. Double-click near start to close, or press Escape to cancel.")
        except Exception as e:
            print(f"[ADD-POLY] Failed to bind for polygon: {e}")
            messagebox.showerror("Error", f"Failed to activate polygon mode: {e}")
    # --- KRAJ IZMENE ---

    def _on_canvas_press(self, event):
        if self.current_drawing_mode != 'rectangle':
            return
            
        self._rect_start = (event.x, event.y)
        if self._rect_id:
            event.widget.delete(self._rect_id)
            self._rect_id = None
        self._rect_id = event.widget.create_rectangle(event.x, event.y, event.x+1, event.y+1, outline='red', width=4)

    def _on_canvas_drag(self, event):
        if self.current_drawing_mode != 'rectangle' or not self._rect_start or not self._rect_id:
            return
        x0, y0 = self._rect_start
        event.widget.coords(self._rect_id, x0, y0, event.x, event.y)

    # --- IZMENA (HIBRIDNI MOD): _on_canvas_release ---
    def _on_canvas_release(self, event):
        ...
        frame_to_crop = self.dialog_frozen_frame
        if frame_to_crop is None:
            self.append_to_chat('Assistant: No frozen frame to crop (unexpected).')
            self._cleanup_drawing_tools()
            return

        fh, fw = frame_to_crop.shape[:2]

        # Map canvas coordinates to original frame coordinates using helper
        mapped0 = self._map_dialog_canvas_to_frame(event.widget, x0, y0, frame_to_crop.shape)
        mapped1 = self._map_dialog_canvas_to_frame(event.widget, x1, y1, frame_to_crop.shape)

        if mapped0 is None or mapped1 is None:
            # Fallback to simple scaling if mapping failed
            try:
                canvas_w = int(event.widget.cget('width'))
                canvas_h = int(event.widget.cget('height'))
                sx = fw / canvas_w
                sy = fh / canvas_h
        ...
        fx1, fy1 = min(fw, fx1), min(fh, fy1)

        if fx1 - fx0 < 10 or fy1 - fy0 < 10:
            self.append_to_chat('Assistant: Selected region too small; try again.')
            self._cleanup_drawing_tools()
            return

        crop = frame_to_crop[fy0:fy1, fx0:fx1]
        
        # --- HIBRIDNI MOD: Sačuvaj isečak privremeno, ne pitaj za ime ---
        self._update_preview_canvas(crop)
        self.pending_dialog_crop = crop # Sačuvaj isečak
        self.append_to_chat("✅ Area saved! Enter name and description on the left, then click 'Save and Teach (Full)'.")
        # --- KRAJ IZMENE ---

        # Clean up drawing mode (ali ne i freeze)
        self._cleanup_drawing_tools()
    # --- KRAJ IZMENE ---

    def _cleanup_drawing_tools(self):
        """Čisti SVE alate za crtanje (rectangle i polygon) sa dialog canvas-a."""
        try:
            if self._rect_id and self.dialog_canvas_widget:
                self.dialog_canvas_widget.delete(self._rect_id)
            self._rect_id = None
            self._rect_start = None
            
            if self.polygon_draw_id and self.dialog_canvas_widget:
                self.dialog_canvas_widget.delete(self.polygon_draw_id)
            if hasattr(self, '_temp_poly_line') and self._temp_poly_line and self.dialog_canvas_widget:
                 self.dialog_canvas_widget.delete(self._temp_poly_line)
            self.polygon_draw_id = None
            self._temp_poly_line = None
            self.polygon_points = []
            ...
            if self.dialog_canvas_widget:
                self.dialog_canvas_widget.config(cursor='')
                self.dialog_canvas_widget.unbind('<ButtonPress-1>')
                self.dialog_canvas_widget.unbind('<B1-Motion>')
                self.dialog_canvas_widget.unbind('<ButtonRelease-1>')
                self.dialog_canvas_widget.unbind('<Double-Button-1>')
                self.dialog_canvas_widget.unbind('<Motion>')
                self.dialog_canvas_widget.unbind('<Escape>')
                # --- IZMENA: Ukloni i Ctrl bindinge ---
                self.dialog_canvas_widget.unbind('<Control-ButtonPress-1>')
                self.dialog_canvas_widget.unbind('<Control-B1-Motion>')
                self.dialog_canvas_widget.unbind('<Control-ButtonRelease-1>')
                # --- KRAJ IZMENE ---
        except Exception as e:
            print(f"Error in drawing cleanup: {e}")
            pass
    
    def _pause_main_canvas(self):
        """Pauzira prikaz na glavnom canvas-u (za vreme rada sa dialog-om)."""
        # Ovo se sada automatski radi preko dialog_update_running zastavice
        pass

    def _map_dialog_canvas_to_frame(self, canvas_widget, x, y, frame_shape):
        """Map (x,y) from dialog canvas coordinates to original frame pixel coordinates.
        Accounts for aspect-fit thumbnailing and centering used when drawing the dialog image.
        Returns (fx, fy) in frame coordinates or None if outside image area.
        """
        try:
            ...
            # If either dimension is zero (not yet realized), fall back to cget
            if canvas_w <= 0 or canvas_h <= 0:
                try:
                    canvas_w = int(canvas_widget.cget('width'))
                    canvas_h = int(canvas_widget.cget('height'))
                except Exception:
                    return None

            # Compute aspect-fit scale and displayed image size
            scale = min(canvas_w / fw, canvas_h / fh)
            disp_w = int(round(fw * scale))
            disp_h = int(round(fh * scale))

            # Offsets because image is centered in canvas
            ...
            fx = int(round((img_x / disp_w) * fw))
            fy = int(round((img_y / disp_h) * fh))
            fx = max(0, min(fx, fw - 1))
            fy = max(0, min(fy, fh - 1))
            return fx, fy
        except Exception as e:
            print(f"[MAP] Error mapping canvas->frame: {e}")
            return None

    def _on_polygon_click(self, event):
        """Dodaje tačku u poligon."""
        if self.current_drawing_mode != 'polygon':
            return

        self.polygon_points.append((event.x, event.y))
        
        if self.polygon_draw_id:
            self.dialog_canvas_widget.delete(self.polygon_draw_id)
        
        if len(self.polygon_points) > 1:
            self.polygon_draw_id = self.dialog_canvas_widget.create_line(
                self.polygon_points, 
                fill='red', width=2
            )

    def _on_polygon_motion(self, event):
        """Iscrtava liniju do kursora miša."""
        if self.current_drawing_mode != 'polygon' or not self.polygon_points:
            return
            
        if hasattr(self, '_temp_poly_line') and self._temp_poly_line:
            self.dialog_canvas_widget.delete(self._temp_poly_line)
            
        last_point = self.polygon_points[-1]
        self._temp_poly_line = self.dialog_canvas_widget.create_line(
            last_point[0], last_point[1], event.x, event.y, 
            fill='red', width=2, dash=(4, 4)
        )

    # --- IZMENA (HIBRIDNI MOD): _on_polygon_close ---
    def _on_polygon_close(self, event):
        """Zatvara poligon i obrađuje isečak sa auto-correct.
        POBOLJSANJA: Auto-snap na blisku pocetnu tacku
        """
        if self.current_drawing_mode != 'polygon' or len(self.polygon_points) < 3:
            self.append_to_chat("System: Polygon needs at least 3 points.")
            return

        # Auto-correct: Pronalazi blisku tacku pocetku
        first_point = self.polygon_points[0]
        CLOSE_TOLERANCE = 30
        closest_idx = None
        min_distance = CLOSE_TOLERANCE
        
        for i in range(1, len(self.polygon_points)):
            p = self.polygon_points[i]
            dist = ((p[0] - first_point[0])**2 + (p[1] - first_point[1])**2) ** 0.5
            if dist < min_distance:
                min_distance = dist
                closest_idx = i
        
        if closest_idx is not None:
            self.polygon_points[closest_idx] = first_point
            self.polygon_points = self.polygon_points[:closest_idx+1]
            self.append_to_chat("Polygon auto-corrected and closed.")
        else:
        ...
        if self.polygon_draw_id:
            self.dialog_canvas_widget.delete(self.polygon_draw_id)
        self.polygon_draw_id = self.dialog_canvas_widget.create_line(
            self.polygon_points, fill='green', width=3
        )
        ...
        frame_to_crop = self.dialog_frozen_frame
        if frame_to_crop is None:
            self.append_to_chat('Assistant: No frozen frame to crop.')
            self._cleanup_drawing_tools()
            return

        fh, fw = frame_to_crop.shape[:2]

        # Map each polygon canvas point to frame coordinates
        frame_points = []
        for p in self.polygon_points:
            mapped = self._map_dialog_canvas_to_frame(self.dialog_canvas_widget, p[0], p[1], frame_to_crop.shape)
            if mapped is None:
                # If any mapping fails, fall back to simple uniform scaling
                try:
                    canvas_w = int(self.dialog_canvas_widget.cget('width'))
                    canvas_h = int(self.dialog_canvas_widget.cget('height'))
                    sx, sy = fw / canvas_w, fh / canvas_h
                    frame_points = [(int(pp[0] * sx), int(pp[1] * sy)) for pp in self.polygon_points]
                    break
                except Exception:
                    self.append_to_chat('Assistant: Could not map polygon coordinates to frame.')
                    self._cleanup_drawing_tools()
                    return
            else:
                frame_points.append((int(mapped[0]), int(mapped[1])))
        frame_points_np = np.array(frame_points, dtype=np.int32)
        
        mask = np.zeros(frame_to_crop.shape[:2], dtype=np.uint8)
        cv2.fillPoly(mask, [frame_points_np], (255))
        
        result_crop = cv2.bitwise_and(frame_to_crop, frame_to_crop, mask=mask)
        
        try:
            b, g, r = cv2.split(result_crop)
            alpha = mask
            final_crop_bgra = cv2.merge((b, g, r, alpha))
            
            rect = cv2.boundingRect(frame_points_np)
            x, y, w, h = rect
            final_crop = final_crop_bgra[y:y+h, x:x+w]
        except Exception as e:
            print(f"[POLY-CROP] Greška pri kreiranju BGRA, vraćam BGR: {e}")
            rect = cv2.boundingRect(frame_points_np)
            x, y, w, h = rect
        ...
        self.pending_dialog_crop = final_crop # Sačuvaj isečak
        self.append_to_chat("✅ Area saved! Enter name and description on the left, then click 'Save and Teach (Full)'.")
        # --- KRAJ IZMENE ---
        
        # Resetuj alate (ali ne i freeze)
        self._cleanup_drawing_tools()
    # --- KRAJ IZMENE ---

    # --- UKLONJENO: _save_visual_sample više nije potreban ---
    # Logika je premeštena u 'save()' funkciju unutar 'start_learning'

    def _update_preview_canvas(self, np_image):
        """Prikazuje CV2 (numpy) sliku na self.dialog_preview_canvas."""
        if self.dialog_preview_canvas is None:
            return
            
        if np_image is None:
            self.dialog_preview_canvas.delete("all")
            self.dialog_preview_photo = None
            return
        ...
        try:
            ...
            if len(np_image.shape) == 3 and np_image.shape[2] == 4:
                img_rgba = cv2.cvtColor(np_image, cv2.COLOR_BGRA2RGBA)
                img_pil = PIL.Image.fromarray(img_rgba)
            elif len(np_image.shape) == 2:
                img_rgb = cv2.cvtColor(np_image, cv2.COLOR_GRAY2RGB)
            ...
            img_pil.thumbnail(preview_size, PIL.Image.Resampling.LANCZOS)
            
            self.dialog_preview_photo = PIL.ImageTk.PhotoImage(image=img_pil)
            
            self.dialog_preview_canvas.delete("all")
            self.dialog_preview_canvas.create_image(
                preview_size[0] / 2,
                preview_size[1] / 2,
                image=self.dialog_preview_photo, 
                anchor=tk.CENTER
            )
        except Exception as e:
            print(f"[PREVIEW] Greška pri ažuriranju pregleda isečka: {e}")
            self.dialog_preview_canvas.delete("all")
            self.dialog_preview_photo = None

    # --- IZMENA (HIBRIDNI MOD + Zahtev 2): 'start_learning' modifikovan ---
    def start_learning(self):
        """
        Otvara "Teach Me" dialog sa HIBRIDNOM logikom.
        Sada koristi isključivo add_clip_embedding (Zahtev 2).
        """
        
        if not self.llm and getattr(self, 'backend', 'local') != 'ollama':
            messagebox.showerror("Error", "LLM not initialized and Ollama backend not available")
            return
        ...
        # --- Leva kolona (Unos teksta) ---
        topic_frame = ttk.LabelFrame(left_column, text="Topic", padding="5")
        topic_frame.pack(fill=tk.X, pady=(0, 10))
        
        name_frame = ttk.Frame(topic_frame)
        name_frame.pack(fill=tk.X)
        ttk.Label(name_frame, text="Name:").pack(side=tk.LEFT)
        name_entry = ttk.Entry(name_frame)
        name_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)
        
        desc_frame = ttk.LabelFrame(left_column, text="Description", padding="5")
        desc_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        desc_text = scrolledtext.ScrolledText(desc_frame, height=5, wrap=tk.WORD)
        desc_text.pack(fill=tk.BOTH, expand=True)
        
        examples_frame = ttk.LabelFrame(left_column, text="Examples (one per line)", padding="5")
        examples_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        examples_text = scrolledtext.ScrolledText(examples_frame, height=5, wrap=tk.WORD)
        examples_text.pack(fill=tk.BOTH, expand=True)
        
        context_frame = ttk.LabelFrame(left_column, text="Learning Context", padding="5")
        context_frame.pack(fill=tk.X, pady=(0, 10))
        
        # --- NOVO: Čuvanje reference na context_var ---
        context_var = tk.StringVar(value="general")
        self.dialog_context_var = context_var # Sačuvaj referencu
        
        rb_general = ttk.Radiobutton(context_frame, text="General Knowledge", 
                       variable=context_var, value="general")
        rb_general.pack(side=tk.LEFT, padx=5)
                       
        rb_visual = ttk.Radiobutton(context_frame, text="Visual Recognition", 
                       variable=context_var, value="visual")
        rb_visual.pack(side=tk.LEFT, padx=5)
                       
        rb_face = ttk.Radiobutton(context_frame, text="Face Recognition", 
                       variable=context_var, value="face")
        rb_face.pack(side=tk.LEFT, padx=5)
        
        # --- Desna kolona (Vizuelni alati) ---
        dialog_canvas = None
        
        if self.is_camera_on:
            image_frame = ttk.LabelFrame(right_column, text="Live Feed", padding="5")
            image_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
            
            # --- ISPRAVKA RASPOREDA (LAYOUT FIX) ---
            # 1. Frejm za dugmad, pakuj DESNO (fill=tk.Y da se rastegne vertikalno)
            image_controls = ttk.Frame(image_frame)
            image_controls.pack(side=tk.RIGHT, fill=tk.Y, padx=5)
            
            # 2. Canvas, pakuj LEVO i pusti ga da se proširi
            dialog_canvas = tk.Canvas(image_frame, width=640, height=480, bg='black')
            dialog_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)
            self.dialog_canvas_widget = dialog_canvas
            # --- KRAJ ISPRAVKE RASPOREDA ---
            
            # Dodaj dugmad u 'image_controls' (isto kao pre)
            freeze_btn = ttk.Button(image_controls, text="Freeze Frame (Ctrl+F)",
                                   command=self.dialog_toggle_freeze)
            freeze_btn.pack(pady=5, fill=tk.X)
            
            fast_add_btn = ttk.Button(image_controls, text="Add Object (Rectangle)",
                                      command=self.start_add_object) 
            fast_add_btn.pack(pady=5, fill=tk.X)

            poly_add_btn = ttk.Button(image_controls, text="Select Area (Polygon)",
                                      command=self.start_polygon_select)
            poly_add_btn.pack(pady=5, fill=tk.X)
            ...
            rb_face.config(state=tk.DISABLED)
            context_var.set("general")
        
        
        # --- IZMENA (HIBRIDNI MOD + Zahtev 2): Funkcija 'save()' ---
        def save():
            """
            "Save and Teach (Full)" - Hibridna logika.
            Čuva tekst I privremeni isečak (ako postoji) koristeći CLIP.
            """
            ...
            frame_to_learn = None
            if self.dialog_is_frozen and self.dialog_frozen_frame is not None:
                frame_to_learn = self.dialog_frozen_frame
            else:
                frame_to_learn = self.last_frame
                
            if frame_to_learn is None and context in ('face', 'visual') and self.pending_dialog_crop is None:
                messagebox.showerror("Error", 
                    f"Cannot learn '{context}'. Camera is off or no frame available (and no crop selected).", 
                    parent=dialog)
                return

            # --- Lokalni podaci ---
            captured_face_crop = None # (GRAY)
            captured_visual_crop = None # (BGR ili BGRA)
            
            # --- Logika Hibridnog Moda ---
            
            if (context == 'visual' or context == 'general') and self.pending_dialog_crop is not None:
                # 1. Prioritet: Koristi isečak ako postoji
                captured_visual_crop = self.pending_dialog_crop
                self.pending_dialog_crop = None # Obriši ga
                print("[LEARN] Koristi se isečak iz 'pending_dialog_crop'.")
            
            elif context == 'face':
                # 2. Kontekst je Lice
                try:
                    gray = cv2.cvtColor(frame_to_learn, cv2.COLOR_BGR2GRAY)
                    faces = []
                    if hasattr(self, 'face_cascade') and self.face_cascade is not None:
                        faces = self.face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=4)
                    
                    if len(faces) != 1:
                        messagebox.showwarning("Face Error", 
                            f"Found {len(faces)} faces in the frame. Please freeze a frame with exactly ONE clear face.", 
                            parent=dialog)
                        return
                    
                    (fx, fy, fw, fh) = faces[0]
                    face_crop_gray = gray[fy:fy+fh, fx:fx+fw]
                    captured_face_crop = cv2.resize(face_crop_gray, (200, 200))
                    self._update_preview_canvas(captured_face_crop)
                    
                except Exception as e:
                    messagebox.showerror("Error", f"Failed to process face: {e}", parent=dialog)
                    return
            
            elif context == 'visual':
                # 3. Kontekst je Vizuelan, ali NEMA isečka. Koristi ceo frejm kao fallback.
                captured_visual_crop = frame_to_learn
                self._update_preview_canvas(captured_visual_crop)
                print("[LEARN] Nema isečka, koristi se ceo frejm kao fallback.")

            
            # --- Kreiranje prompta za LLM (isto kao pre) ---
            learning_prompt = f"""Please analyze and learn about: {name}
Description:
{desc}
Context: {context}
"""
            if context in ('face', 'visual', 'general'):
                learning_prompt += f"\nThis knowledge includes a visual component."
            if examples:
                learning_prompt += "\nExamples:\n" + "\n".join(f"- {ex}" for ex in examples)
            learning_prompt += """
Instructions:
- Provide a concise, user-facing SUMMARY (1-3 sentences) that can be shown directly to users.
- Provide a short list of likely user questions and brief answers (3-5 items).
- Do NOT output chain-of-thought, internal reasoning, or step-by-step thinking.
- Output should be readable plain text suitable for direct display.
Please provide the SUMMARY followed by the QA examples:
"""
            
            try:
                # --- Generisanje razumevanja (isto kao pre) ---
                understanding = ""
                if getattr(self, 'backend', 'local') == 'ollama':
                    try:
                        response_text = self.ollama_generate(learning_prompt, model=self.ollama_model, max_tokens=400)
                        if response_text and not response_text.startswith('[OLLAMA'):
                            understanding = response_text.strip()
                        # --- FIX: Add fallback if response is empty ---
                        if not understanding:
                            understanding = f"Learned: {name}. Description: {desc}"
                            print(f"[LEARN] Using fallback understanding for {name}")
                    except Exception as e:
                        print(f"[LEARN] Ollama generation failed: {e}")
                        # Fallback on error
                        understanding = f"Learned: {name}. Description: {desc}"
                else:
                    pass 
                
                # --- Čuvanje u bazu znanja ---
                # knowledge_entry se sada kreira unutar add_clip_embedding ili add_face_sample
                
                # --- Čuvanje vizuelnih/face uzoraka (Hibridna logika) ---
                
                if context == 'face' and captured_face_crop is not None:
                    # 1. Sačuvaj Lice (LBPH)
                    try:
                        added = self.add_face_sample(name, captured_face_crop)
                        if name not in self.knowledge_base:
                             self.knowledge_base[name] = {}
                        self.knowledge_base[name].update({
                            "description": desc, "examples": examples, "context": context,
                            "understanding": understanding, "learned_on": datetime.now().isoformat()
                        })
                        self.knowledge_base[name].setdefault('notes', []).append('Face sample saved' if added else 'Face sample failed')
                        self.save_knowledge_base()
                    except Exception as e:
                        print(f"Error processing face sample: {e}")
                        self.knowledge_base[name].setdefault('notes', []).append('Face sample processing failed')
                
                elif (context == 'visual' or context == 'general') and captured_visual_crop is not None:
                    # 2. NOVO: Sačuvaj isečak (Rectangle/Polygon) koristeći CLIP
                    try:
                        # Konvertuj BGRA u BGR ako je potrebno
                        if captured_visual_crop.shape[2] == 4:
                             img_bgr = cv2.cvtColor(captured_visual_crop, cv2.COLOR_BGRA2BGR)
                        else:
                             img_bgr = captured_visual_crop
                        
                        # add_clip_embedding sada takođe čuva u knowledge_base
                        added = self.add_clip_embedding(name, desc, img_bgr)
                        
                        # Ažuriraj LLM razumevanje (ako je generisano)
                        if name in self.knowledge_base:
                            self.knowledge_base[name]['understanding'] = understanding
                            self.save_knowledge_base()
                        
                    except Exception as e:
                        print(f"Error adding visual (CLIP) crop sample: {e}")
                
                elif context == 'visual' and captured_visual_crop is not None:
                    # 3. Fallback: Sačuvaj ceo frejm (CLIP)
                    try:
                        added = self.add_clip_embedding(name, desc, captured_visual_crop)
                        if name in self.knowledge_base:
                            self.knowledge_base[name]['understanding'] = understanding
                            self.save_knowledge_base()
                    except Exception as e:
                        print(f"Error adding visual (CLIP) full frame sample: {e}")

                confirmation = f"Assistant: I've learned about {name}. Summary:\n{understanding}"
                self.append_to_chat(confirmation)
                self.dialog_cleanup_on_close(dialog)
                
            except Exception as e:
                messagebox.showerror("Error", f"Failed to process learning: {str(e)}", parent=dialog)
        
        # --- Kraj 'save()' funkcije ---

        ttk.Button(button_frame, text="Cancel", 
                  command=lambda: self.dialog_cleanup_on_close(dialog)).pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="Save and Teach (Full)", 
                  command=save).pack(side=tk.RIGHT, padx=5)
        
        dialog.bind('<Control-f>', lambda e: self.dialog_toggle_freeze())
        dialog.bind('<Control-F>', lambda e: self.dialog_toggle_freeze())
        dialog.bind('<Escape>', lambda e: self.dialog_cleanup_on_close(dialog))
        dialog.protocol("WM_DELETE_WINDOW", lambda: self.dialog_cleanup_on_close(dialog))
        
        if self.is_camera_on and dialog_canvas is not None:
            # Pause main window rendering and clear main canvas to avoid duplicate views
            self.dialog_update_running = True
            try:
                ...
                pass
            # Start updating the dialog feed (dialog owns the displayed frame while open)
            self.update_dialog_feed(dialog, dialog_canvas)
    # --- KRAJ IZMENE 'start_learning' ---

    def dialog_toggle_freeze(self, freeze=None):
        """Freezes or unfreezes the video feed in the Teach Me dialog."""
        if freeze is None:
            self.dialog_is_frozen = not self.dialog_is_frozen
        else:
            self.dialog_is_frozen = bool(freeze)
            
        if self.dialog_is_frozen:
            # --- IZMENA: Koristi sinhronizovani frejm ---
            if self.last_raw_frame_processed is not None:
                self.dialog_frozen_frame = self.last_raw_frame_processed.copy()
                self.append_to_chat("System: Teach dialog frame frozen.")
            elif self.last_frame is not None:
                # Fallback ako last_raw_frame_processed još nije postavljen
                self.dialog_frozen_frame = self.last_frame.copy()
                self.append_to_chat("System: Teach dialog frame frozen (fallback).")
            # --- KRAJ IZMENE ---
            else:
                self.dialog_is_frozen = False
        else:
            self.dialog_frozen_frame = None
            self._cleanup_drawing_tools()
            self.append_to_chat("System: Teach dialog frame unfro+zen.")

    def update_dialog_feed(self, dialog, dialog_canvas):
        """Samo ažurira canvas u 'Teach Me' dialogu."""
        if not self.dialog_update_running:
            return
        ...
        try:
            if self.dialog_is_frozen and self.dialog_frozen_frame is not None:
                if self.current_drawing_mode is not None:
                    frame_to_show = self.dialog_frozen_frame
                elif self.last_processed_frame is not None:
                    frame_to_show = self.last_processed_frame
                else:
                    frame_to_show = self.dialog_frozen_frame
            else:
                frame_to_show = self.last_processed_frame
            ...
            if frame_to_show is not None:
                canvas_w = dialog_canvas.winfo_width()
                canvas_h = dialog_canvas.winfo_height()
                
                if canvas_w < 10 or canvas_h < 10:
                    dialog.after(30, self.update_dialog_feed, dialog, dialog_canvas)
                    return

                if len(frame_to_show.shape) == 3 and frame_to_show.shape[2] == 4:
                     frame_rgb = cv2.cvtColor(frame_to_show, cv2.COLOR_BGRA2RGBA)
                ...
                self.dialog_photo = PIL.ImageTk.PhotoImage(image=frame_pil)
                
                dialog_canvas.delete("all")
                dialog_canvas.create_image(
                    canvas_w / 2, 
                    canvas_h / 2, 
                    image=self.dialog_photo, 
                    anchor=tk.CENTER
                )
                
                # Ponovo iscrtaj alate ako postoje
                if self.current_drawing_mode == 'rectangle' and self._rect_id:
                    if self._rect_start:
                        try:
                            # Pokušaj da dobiješ koordinate
                            coords = self.dialog_canvas_widget.coords(self._rect_id)
                            if coords:
                                x0, y0 = self._rect_start
                                x1, y1 = coords[2:]
                                self._rect_id = self.dialog_canvas_widget.create_rectangle(x0, y0, x1, y1, outline='red', width=4)
                        except Exception:
                             # self._rect_id je verovatno obrisan, kreiraj ponovo
                             x0, y0 = self._rect_start
                             self._rect_id = self.dialog_canvas_widget.create_rectangle(x0, y0, x0+1, y0+1, outline='red', width=4)

                elif self.current_drawing_mode == 'polygon' and self.polygon_points:
                    if self.polygon_draw_id:
                        self.dialog_canvas_widget.delete(self.polygon_draw_id)
                    # Proveri da li je poligon zatvoren
                    is_closed = len(self.polygon_points) > 1 and self.polygon_points[0] == self.polygon_points[-1]
                    self.polygon_draw_id = self.dialog_canvas_widget.create_line(
                        self.polygon_points, fill='red', width=3 if is_closed else 2
                    )
            
            dialog.after(30, self.update_dialog_feed, dialog, dialog_canvas)
            
        except Exception as e:
            # Smanji količinu logovanja za TclError
            if "invalid command name" not in str(e):
                print(f"[DIALOG-FEED] Greška u petlji: {e}")
            self.dialog_update_running = False

    # --- IZMENA (HIBRIDNI MOD): dialog_cleanup_on_close ---
    def dialog_cleanup_on_close(self, dialog):
        """Cleans up all resources related to the Teach Me dialog."""
        self.dialog_update_running = False
        self.dialog_is_frozen = False
        self.dialog_frozen_frame = None
        ...
        # --- NOVO: Obriši privremene podatke ---
        self.pending_dialog_crop = None
        self.dialog_context_var = None
    ...
    def process_and_draw_frame(self, frame):
        """
        Runs all detection and recognition on a single frame
        and returns the frame with boxes drawn on it.
        """
